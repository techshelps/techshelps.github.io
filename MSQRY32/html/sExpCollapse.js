...
function ExpandAll()
{
	...
	if (sCaption==L_EXPAND_TEXT)
		{
		sAction="expanded";
		sImage="mk:@msitstore:msohlp11.chm::/html/bluedrop.gif"
		document.all("picHeader").src="mk:@msitstore:msohlp11.chm::/html/collapsetri.gif";
		document.all("ExpandAll").innerHTML=L_COLLAPSE_TEXT;
		}
	else
		{
		sAction="collapsed";
		sImage="mk:@msitstore:msohlp11.chm::/html/blueup.gif"
		document.all("picHeader").src="mk:@msitstore:msohlp11.chm::/html/expandtri.gif";
...
function Outline2()
{
	...
	if(document.all(elemPos+incr).className=="collapsed")
	{
		...
		document.all(elemPos+incr).className="expanded"
		document.all(elemPos+1).src="mk:@msitstore:msohlp11.chm::/html/bluedrop.gif";
		if(open.tagName=="IMG"){open.src="mk:@msitstore:msohlp11.chm::/html/bluedrop.gif";}
		if(open.tagName=="B")
			{
			if(open.parentElement.all.tags("IMG").length != 0)
				{open.parentElement.all.tags("IMG").item(0).src="mk:@msitstore:msohlp11.chm::/html/bluedrop.gif";}
			}
	}
	else if(document.all(elemPos+incr).className=="expanded")
	{
		document.all(elemPos+incr).className="collapsed"
		document.all(elemPos+1).src="mk:@msitstore:msohlp11.chm::/html/blueup.gif";
		if(open.tagName=="IMG"){open.src="mk:@msitstore:msohlp11.chm::/html/blueup.gif";}
		if(open.tagName=="B")
			{
			if(open.parentElement.all.tags("IMG").length != 0)
				{open.parentElement.all.tags("IMG").item(0).src="mk:@msitstore:msohlp11.chm::/html/blueup.gif";}
			}
	}
	else
	{
...
//elemPos = window.event.srcElement.sourceIndex;
	...
	if (defSpan && document.readyState == "complete" && theIFRAME.document.readyState == "complete"){
			...
			document.all(elemPos+1).src="mk:@msitstore:msohlp11.chm::/html/bluedrop.gif";
			document.all(elemPos).src="mk:@msitstore:msohlp11.chm::/html/bluedrop.gif";
		}
    		else{
			defSpan.style.display = "none";
			defSpan.parentElement.style.display = "none";
			document.all(elemPos+1).src="mk:@msitstore:msohlp11.chm::/html/blueup.gif";
			//if(oSource.tagName=="IMG"){open.src="mk:@msitstore:msohlp11.chm::/html/blueup.gif";}
    		}
	}
}
